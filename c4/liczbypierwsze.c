...
#pragma omp parallel for shared(a) private(k)
  ...
  lpodz = llpier;
#pragma omp parallel for private(k, liczba, reszta)                            \
    shared(pierwsze, pierwsze_thread_private_arr, llpier_thread_private_arr)
  for (liczba = S + 1; liczba <= N; liczba++) {
    ...
    if (reszta != 0) {
      ...
      pierwsze_thread_private_arr[thread_num]
                                 [llpier_thread_private_arr[thread_num]++] =
                                     liczba;
    }
  }
